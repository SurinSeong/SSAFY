# 문자열 메서드 체이닝
text = 'heLLo, woRld!'
new_text = text.swapcase().replace('l', 'z') # 1. text.swapcase() == 'HEllO, WOrLD!' -> 2. 'HEllO, WOrLD!'.replace('l', 'z') == 'HEzzO, WOrLD!'
...
result = numbers.copy().sort() # 이렇게 하면 안된다. sort()는 반환을 하는 것이 아님.
print(result)  # None (sort()는 None을 반환하므로 체이닝이 중단됨)
print(numbers)  # [3, 1, 4, 1, 5, 9, 2] (원본은 변경되지 않음)

# 잘못된 체이닝 방식 2
result = numbers.append(7).extend([8, 9])  # AttributeError (in numbers.append(7)) -> .append() 반환값 없음.
