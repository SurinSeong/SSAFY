...
# 자식 클래스
class Child(ParentA, ParentB): # super의 시작점.
    # 생성자 메서드
    def __init__(self):
        # 부모 클래스의 인스턴스 변수 상속
        super().__init__() # super() : ParentA
        # 자기 자신의 인스턴스 변수
        self.value_c = 'Child'

    def show_value(self):
        super().show_value() # super()의 show_value 메서드 호출
...
print(child.value_c)
print(child.value_a)
print(child.value_b)    # MRO에 의해 ParentA의 value_a만 상속 받음. 하지만 ParentA에 super()를 사용해 상위 클래스의 인스턴스 변수를 상속 받는다면 value_b가 출력된다. 
